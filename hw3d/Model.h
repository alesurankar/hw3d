...
public:
	...
	void Draw(Graphics& gfx, DirectX::FXMMATRIX accumulatedTransform) const noexcept(!NDEBUG)
	{
...
class Node
{
	friend class Model;
public:
	Node(std::vector<Mesh*> meshPtrs, const DirectX::XMMATRIX& transform) noexcept(!NDEBUG)
		:
...
private:
	void AddChild(std::unique_ptr<Node> pChild) noexcept(!NDEBUG)
	{
		assert(pChild);
